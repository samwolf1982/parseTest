...
add_action( 'myprefix_cron_a_free', 'run_a_free' );
	function cron_a_free( $schedules ) {
	...
	if ( ! wp_next_scheduled( 'myprefix_cron_a_free' ) ) {
			wp_schedule_event( time()+$GLOBALS['interval_next'], 'weekly', 'myprefix_cron_a_free' );
			$GLOBALS['interval_next']+=$GLOBALS['interval_add'];
	}
function run_a_free()
{
	# code...
   $data = array('path'=>'http://anti-free.ru/forum/showthread.php?t=58815&page=132',
              'count'=>'1'
              );
          $url='http://imageplugin.dev/wp-content/plugins/megaparse/php5/scanx.php';
